...
import { getSet, getCubableCardByName } from "./data";
import { Cube } from '../common/src/types/cube';
import { GameOptions, StartOptions } from '../common/src/types/game';
...
function controlCubeSettingsAndTransformList(cube: Cube, seats: number, type: string) {
  ...
  for (let cardName of list)
    if (!getCubableCardByName(cardName))
      bad.push(cardName);
...
export const deck = (deck: { [x: string]: any; }, pool: Card[]) => {
...
export const prepareGame = ({ seats, type, sets, cube, isPrivate, modernOnly = true, chaosPacksNumber, totalChaos = true }: GameOptions) => {
  ...
  case "sealed":
    if (sets) {
      ...
      throw Error('Tried to create a regular draft while sets not defined');
    }
    break;
  case "cube draft":
  case "cube sealed":
    if (cube !== undefined) {
      ...
      throw Error('Tried to create a cube draft while cube not defined');
    }
    break;
  case "chaos draft":
  case "chaos sealed":
...
export const start = ({ addBots, useTimer, timerLength, shufflePlayers }: StartOptions) => {
